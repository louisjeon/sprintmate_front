...
const TeamDetailPage = () => {
  ...
  const handleCreateProject = async () => {
    try {
      ...
      const newProject = await apiCreateProject(teamId, projectCreateRequest); // Updated
      ...
      setTeam((prev) => ({
        ...
        projects: [
          ...prev.projects,
          { projectId: newProject.projectId, projectName: newProjectName }, // Adjust based on actual needs
        ],
      }));
  ...
  if (!team) {
    return (
      <p className="text-center mt-4 text-gray-500">팀 정보를 불러오는 중...</p>
    );
  }

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">{team.teamName}</h1>
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex items-center text-sm text-gray-600 gap-4 mb-4">
          <div className="flex items-center gap-1">
            <CalendarClock size={16} />
            <span>생성일: {new Date(team.createdAt).toLocaleDateString()}</span>
          </div>
        </div>
          ...
          <ul className="divide-y divide-gray-200 border rounded-md">
            {team.teamMembers.map((member) => (
              ...
              >
                ...
                <span className="text-gray-500">{member.memberRoleName}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        {/* 프로젝트 목록 */}
        <div className="mt-6">
          <h3 className="text-xl font-semibold mb-3">프로젝트 목록</h3>
          <ul className="divide-y divide-gray-200 border rounded-md">
            {team.projects.map((project) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    navigate(`/teams/${teamId}/projects/${project.projectId}`)
                  }
